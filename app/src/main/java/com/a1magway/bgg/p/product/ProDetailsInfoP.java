...
public class ProDetailsInfoP extends BasePresenter<IProDetailsInfoV> {
    ...
    private Commodity mSelectedCommodityDialog;//dialog选择的属性
...
//        mView.setSelectedSku(mProduct.getSkuList().get(0).getSpecsMap());
    ...
    /**
     * 当选择了款式和样式之后，回调回来选择了完整的规格组合
     */
    public void selectedCompleteSkuDilog(final Map<String, String> selectedMap) {
        // 遍历服务器返回的所有规格组合的货品
        Observable.fromIterable(mProductLazy.get().getSkuList())
                .doOnNext(
                        new Consumer<Commodity>() {
                            ...
                            public void accept(
                                ...
                                if (commodity.getSpecsMap().equals(selectedMap)) {
                                    // 匹配到对应的货品
                                    showCommodityDialog(commodity);
                                    mView.showCommDialogCallBackData(commodity);
                                }
                            }
                        })
                .subscribe(new Observer<Commodity>() {
    ...
    private void showCommodityDialog(Commodity commodity) {
        MLog.i(TAG, "显示货品信息:" + commodity);

        mSelectedCommodityDialog = commodity;
//        mView.showCoverBanner(commodity.getSkuCover());
//        mView.showStockTxt(
//                String.format(
//                        getContext().getString(R.string.product_detail_format_stock),
...
//        showPrice(commodity);
    ...
    public Serializable getSelectedCommodityDialog() {
        if (!mView.isSelectedWholeSkuDialog()) {
            // 未选择完整的规格组合返回null
            return null;
        }
        return mSelectedCommodityDialog;
    }

    //重置选择框上次选中的属性指
    public void setmSelectedCommodityDialog() {
        this.mSelectedCommodityDialog = null;
    }

    public void setmSelectedCommodityDialogInside(Commodity mSelectedCommodity){
        this.mSelectedCommodityDialog=mSelectedCommodity;
    }
